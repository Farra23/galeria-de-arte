...
1. **Detiene el servicio unos segundos.** Al cerrar limpio, SQLite consolida el diario de
   ...
   levantar igual.
2. Copia `app.db` (+ `-wal` / `-shm` si existen) y las imágenes de obras.
...
6. Escribe una línea en `backups.log` con el resultado — incluidos los fallos.
...
# 3. Copiar la carpeta nueva sobre C:\GaleriaACATRAS\app
#    (Datos\ y wwwroot\uploads\ NO se tocan: son los datos del cliente)

# 4. Reinstalar el servicio para que tome los binarios nuevos
...
| El último backup tiene más de 2 días | El backup automático dejó de correr. Revisar `backups.log`. |
